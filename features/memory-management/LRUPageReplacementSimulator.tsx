...
import { getLruSimulation } from '../../services/lruPageReplacementService';
...
const LRUPageReplacementSimulator: React.FC = () => {
    ...
    const runSimulation = useCallback(() => {
        ...
        if (pages.length > 0 && frameCount > 0) {
            const simData = getLruSimulation(pages, frameCount);
    ...
    const lruStack = stepData?.state.lruStack || [];
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
                ...
                <Card title="Recency Stack (LRU -> MRU)">
                    <div className="flex justify-start items-center space-x-1 p-2">
                        {lruStack.map((page: string, index: number) => (
                            <div key={index} className={`px-3 py-1 rounded-md font-mono text-sm ${index === 0 ? 'bg-yellow-800' : 'bg-background-elevated'}`}>
                                {page}
                            </div>
                        ))}
                    </div>
...
export default LRUPageReplacementSimulator;
