...
import { getOptimalSimulation } from '../../services/optimalPageReplacementService';
...
const OptimalPageReplacementSimulator: React.FC = () => {
    ...
    const runSimulation = useCallback(() => {
        ...
        if (pages.length > 0 && frameCount > 0) {
            const simData = getOptimalSimulation(pages, frameCount);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
                <Card title="Memory Frames">
                    <div className="flex justify-center items-center space-x-2 p-4 min-h-[120px]">
                        {stepData?.state.frames.map((page: string | null, index: number) => (
                             <div key={index} className={`w-20 h-20 flex items-center justify-center text-2xl font-bold rounded-md border-2
                                ...
                                ${stepData?.state.lastEvent?.replaced === page ? 'border-yellow-500' : ''}
                                `}>
                                {page ?? '-'}
                            </div>
                        ))}
                 ...
                 <Card title="Page Reference String (with Lookahead)">
                    <div className="flex justify-center items-center space-x-1 p-2 flex-wrap">
                        {stepData?.state.pages.map((page: string, index: number) => (
                            <div key={index} className={`px-3 py-1 rounded-md font-mono 
                                ${index === stepData.state.pageIndex ? 'bg-accent-primary text-white' : 
                                index > stepData.state.pageIndex ? 'bg-background-elevated/50 text-text-tertiary' : 'bg-background-elevated'}`}>
                                {page}
                            </div>
                        ))}
                    </div>
...
export default OptimalPageReplacementSimulator;