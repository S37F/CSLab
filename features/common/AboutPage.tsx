...
interface AboutPageProps {
    ...
    // FIX: Changed React.ReactNode to React.ReactElement to allow cloning with new props. The type was still too generic.
    // Making it more specific to `React.ReactElement<{ className?: string }>` to solve the overload error on cloneElement.
...
const AboutPage: React.FC<AboutPageProps> = ({ stats, categories, groupedAlgorithms, categoryIcons }) => {
    ...
    // FIX: Explicitly typed variants to prevent type inference issues.
    ...
    // FIX: Explicitly typed variants to fix 'ease' property type error.
                     ...
                     >
                        {categories.map(category => (
                            <AlgorithmCategoryCard 
                                key={category}
                                // FIX: Removed type assertion which was causing an error. Prop type was corrected.
