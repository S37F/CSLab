...
import { getSjfSchedulingSimulation } from '../../services/sjfSchedulingService';
...
const SJFSchedulingSimulator: React.FC = () => {
  ...
  const runSimulation = useCallback(() => {
    ...
    if (validProcesses.length > 0) {
      const simData = getSjfSchedulingSimulation(validProcesses, isPreemptive);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2">
        <Card title={`Gantt Chart - ${isPreemptive ? 'Preemptive (SRTF)' : 'Non-Preemptive'}`}>
          ...
          <div className="flex space-x-2 mt-4">
           ...
           <div className="mt-4 space-y-2">
                <label className="flex items-center cursor-pointer">
                    ...
                    <div className="ml-3 text-text-secondary font-medium">Preemptive (SRTF)</div>
                </label>
            </div>
...
export default SJFSchedulingSimulator;
