...
import { getRoundRobinSimulation } from '../../services/roundRobinService';
...
const RoundRobinSimulator: React.FC = () => {
  const [processes, setProcesses] = useState<Partial<Process>[]>([
    { id: 'P0', arrivalTime: 0, burstTime: 10 },
    { id: 'P1', arrivalTime: 1, burstTime: 5 },
  ...
  const runSimulation = useCallback(() => {
    ...
    if (validProcesses.length > 0 && timeQuantum > 0) {
      const simData = getRoundRobinSimulation(validProcesses, timeQuantum);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2">
        <Card title={`Gantt Chart (Time Quantum: ${timeQuantum})`}>
         ...
         <Card title="Metrics" className="mt-6">
            <div className="font-mono text-sm grid grid-cols-2 gap-4">
                ...
                <div className="flex justify-between">
                    ...
                    <span className="text-text-primary">{stepData?.state.completedCount} / {processes.length}</span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="space-y-2">
            ...
            {processes.map((p, i) => (
              <div key={i} className="grid grid-cols-4 gap-2 items-center">
                <Input className="font-mono col-span-1" value={p.id} readOnly />
             ...
             <div className="pt-2">
                <label className="text-sm font-medium text-text-secondary mb-1 block">Time Quantum</label>
                <Input type="number" min="1" value={timeQuantum} onChange={e => setTimeQuantum(parseInt(e.target.value) || 1)} className="font-mono"/>
            </div>
          </div>
