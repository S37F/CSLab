...
import { getBubbleSortSimulation } from '../../services/bubbleSortService';
...
const BubbleSortSimulator: React.FC = () => {
  const [inputValue, setInputValue] = useState<string>('8, 3, 5, 1, 4, 2');
  const [array, setArray] = useState<number[]>([8, 3, 5, 1, 4, 2]);
  const [simulation, setSimulation] = useState<AlgorithmSimulation | null>(null);
  const [currentStep, setCurrentStep] = useState<number>(0);
  const [isPlaying, setIsPlaying] = useState<boolean>(false);
  const [speed, setSpeed] = useState<number>(50); // 50 is middle value for slider
  ...
  const runSimulation = useCallback(() => {
    ...
    if (parsedArray.length > 0) {
      ...
      const simData = getBubbleSortSimulation(parsedArray);
  ...
  const getBarColor = (index: number) => {
    if (stepData?.state.sortedIndices.includes(index)) return 'bg-accent-success';
    if (stepData?.state.highlights.swapping?.includes(index)) return 'bg-accent-warning';
...
export default BubbleSortSimulator;
