...
import { getSelectionSortSimulation } from '../../services/selectionSortService';
...
const SelectionSortSimulator: React.FC = () => {
  const [inputValue, setInputValue] = useState<string>('64, 25, 12, 22, 11');
  ...
  const runSimulation = useCallback(() => {
    ...
    if (parsedArray.length > 0) {
      ...
      const simData = getSelectionSortSimulation(parsedArray);
  ...
  const getBarColor = (index: number) => {
    ...
    if (index < (stepData?.state.sortedUntilIndex || 0)) return 'bg-accent-success';
    if (highlights.swapping?.includes(index)) return 'bg-accent-warning';
    if (highlights.min?.includes(index)) return 'bg-yellow-500';
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex space-x-2">
                <Input value={inputValue} onChange={(e) => setInputValue(e.target.value)} className="font-mono" />
            ...
            <Slider label="Speed" min="1" max="100" value={speed} onChange={(e) => setSpeed(Number(e.target.value))} />
          </div>
        </Card>
        <Card title="Metrics">
            {stepData && (
...
export default SelectionSortSimulator;
